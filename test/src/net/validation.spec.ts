...
import { Validation } from '../../../dist/net/validation.js';
import { Wallet } from '../../../dist/chain/wallet.js';
...
class TestValidation {
  ...
  static async before() {
    ...
    process.env.I2P_SAM_TCP_HOST = '172.19.75.12';
    ...
    process.env.I2P_SAM_FORWARD_TCP_HOST = process.env.I2P_SAM_FORWARD_TCP_HOST || '172.19.75.1';
    process.env.I2P_SAM_LISTEN_TCP_HOST = process.env.I2P_SAM_LISTEN_TCP_HOST || '0.0.0.0';
    process.env.DEBUG_PERFORMANCE = '1';

    const ___dirname: string = path.dirname(import.meta.url.replace(/^file:\/\//, ''));
    TestValidation.config = await Config.make({ path_app: ___dirname });
