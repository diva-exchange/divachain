...
class TestServerI2P {
  ...
  static async before(): Promise<void> {
    ...
    process.env.I2P_SAM_LISTEN_ADDRESS = process.env.I2P_SAM_LISTEN_ADDRESS || '0.0.0.0';
    process.env.I2P_SAM_LISTEN_FORWARD_HOST = process.env.I2P_SAM_LISTEN_FORWARD_HOST || '172.19.75.1';
    ...
    return new Promise((resolve) => {
      const i = setInterval(async () => {
        if (TestServerI2P.mapConfigServer.size === TestServerI2P.mapServer.size) {
          ...
          console.debug('Waiting for 30 seconds - so I2P can build some tunnels...');
  ...
  async transactionTestLoad() {
    ...
    await TestServerI2P.wait(80000);
  }
  ...
  async stressMultiTransaction() {
    ...
    for (let _i = 0; _i < _outer; _i++) {
      ...
      await TestServerI2P.wait(1 + Math.floor(Math.random() * 200));
    }

    console.debug('waiting 120s for a possible sync');
    // wait for a possible sync
