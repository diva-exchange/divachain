...
import { CommitStruct } from '../p2p/message/commit';
import { Logger } from '../logger';

export class CommitPool {
  list: { [id: string]: Array<CommitStruct> };

  constructor() {
    this.list = {};
  }

  add(commit: CommitStruct): void {
    this.list[commit.hash] ? this.list[commit.hash].push(commit) : (this.list[commit.hash] = [commit]);
  }
